...
export const searchQuery = (searchItem) => {
  const query = `*[_type == 'pin' && category match '${searchItem}*' || title match '${searchItem}*' || about match '${searchItem}*']{
    image{
      asset -> {
        url
      }
...
export const pinDetailMorePinQuery = (pin) => {
  const query = `*[_type == "pin" && category match '${pin.category}' && _id != '${pin._id}' ]{
    image{
      asset->{
        url
      }
